...
def get_activities(pid: int, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
...
def get_history(pid: int, aid: int, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
